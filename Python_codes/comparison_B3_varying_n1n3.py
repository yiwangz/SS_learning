...
def simulation(seed_base):
    ...
    cost_tune = np.zeros(len(n1_vec))
    lam2_tune = np.zeros(len(n1_vec))
    lam3_tune = np.zeros(len(n1_vec))
    cost_opt = np.zeros(len(n1_vec))
    lam2_opt = np.zeros(len(n1_vec))
    lam3_opt = np.zeros(len(n1_vec))
    acc_all_tune = np.zeros(len(n1_vec))
    acc_all_opt = np.zeros(len(n1_vec))
    acc_all_1CV = np.zeros(len(n1_vec))
    acc_all_allCV = np.zeros(len(n1_vec))
    acc_indpt_tune = np.zeros(len(n1_vec))
    acc_indpt_opt = np.zeros(len(n1_vec))
    acc_indpt_1CV = np.zeros(len(n1_vec))
    acc_indpt_allCV = np.zeros(len(n1_vec))
    evf_B1_tune = np.zeros(len(n1_vec))
    evf_B1_opt = np.zeros(len(n1_vec))
    evf_B1_1CV = np.zeros(len(n1_vec))
    evf_B1_allCV = np.zeros(len(n1_vec))
    evf_B2_tune = np.zeros(len(n1_vec))
    evf_B2_opt = np.zeros(len(n1_vec))
    evf_B2_1CV = np.zeros(len(n1_vec))
    evf_B2_allCV = np.zeros(len(n1_vec))
    evf_B3_tune = np.zeros(len(n1_vec))
    evf_B3_opt = np.zeros(len(n1_vec))
    evf_B3_1CV = np.zeros(len(n1_vec))
    evf_B3_allCV = np.zeros(len(n1_vec))
    ts_tune = np.zeros(len(n1_vec))
    ts_opt = np.zeros(len(n1_vec))
    ts_1CV = np.zeros(len(n1_vec))
    ts_allCV = np.zeros(len(n1_vec))
    time_1CV = np.zeros(len(n1_vec))
    time_allCV = np.zeros(len(n1_vec))
    time_tune = np.zeros(len(n1_vec))
    ...
    for index in range(len(n1_vec)):
        ...
        model1 = GridSearchCV(svm.SVC(kernel='linear'), tuned_paras, cv=5, scoring='accuracy', fit_params={'sample_weight': B1/pall[:n1_vec[index]]})
        model1.fit(X1, A1)
