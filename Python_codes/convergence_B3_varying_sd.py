...
sd1 = 0.1
sd2 = 0.5
sd3_vec = [0.1,0.5,1.0,2.0] ##varying B2 sd
...
def simulation(seed_base):
    ...
    cost_tune = np.full(len(sd3_vec), np.nan)
    lam2_tune = np.full(len(sd3_vec), np.nan)
    lam3_tune = np.full(len(sd3_vec), np.nan)
    cost_opt = np.full(len(sd3_vec), np.nan)
    lam2_opt = np.full(len(sd3_vec), np.nan)
    lam3_opt = np.full(len(sd3_vec), np.nan)
    acc_all_tune = np.full(K*len(sd3_vec), np.nan)
    acc_indpt_tune = np.full(K*len(sd3_vec), np.nan)
    obj_tune = np.full(K*len(sd3_vec), np.nan)
    ts_tune = np.full(K*len(sd3_vec), np.nan)
    acc_all_opt = np.full(K*len(sd3_vec), np.nan)
    acc_indpt_opt = np.full(K*len(sd3_vec), np.nan)
    obj_opt = np.full(K*len(sd3_vec), np.nan)
    ts_opt = np.full(K*len(sd3_vec), np.nan)
