...
class dataGeneration:
    ...
    def sim2(self, n1, n2, d, coef1, sd1, dist='uniform'):
        if d < 4:
            print('Too small dimension of features.')
        else:
    ...
    def sim3(self, n1, n2, n3, d, coef1, coef2, sd1, sd2, dist='uniform'):
        if d < 4:
            print('Too small dimension of features.')
        else:
            ## features X1, X2, Xindpt
    ...
    def p(self, studySetting = 'trial'):
        if studySetting == 'trial':
            ...
            for i in range(self.m):
                ...
                pall = np.concatenate((pall, logReg.predict_proba(Xtemp)[::, 1]))
...
## Self training OWL
class STowlLinear:
    def __init__(self, Xall, Aall, Ball, m, dataLabel, pall):
        self.Xall = Xall
        self.Aall = Aall
        self.Ball = Ball
        self.m = m
    ...
    def iniFit(self, cost):
        index1 = [item for sublist in np.where(self.dataLabel == 0) for item in sublist]
        indexRemain = [item for sublist in np.where(self.dataLabel != 0) for item in sublist]
        ...
        Xremain = self.Xall[indexRemain,:]
        Aremain = self.Aall[indexRemain]
        Bremain = self.Ball[indexRemain]
        premain = self.pall[indexRemain]
    ...
    def fit(self, lam, itermax=50, itertol=1e-4, track=True, tuning='SSE', model='linear', method='fs', CI=0.95, scale=False):
        if not hasattr(self, 'pred_ini'):
            ...
            X_aux = np.concatenate((self.Xall, self.Xremain), axis=0)
            ...
            for j in range(self.m-1):
                Btemp = self.Ball[index[j]]
                ptemp = self.pall[index[j]]
            ...
            while (iter < itermax and rel_obj >= itertol):
                A_aux = np.concatenate((self.Aall, predRemainK))
