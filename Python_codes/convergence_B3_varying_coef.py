...
def simulation(seed_base):
    ...
    cost_tune = np.full(len(coef3_vec), np.nan)
    lam2_tune = np.full(len(coef3_vec), np.nan)
    lam3_tune = np.full(len(coef3_vec), np.nan)
    cost_opt = np.full(len(coef3_vec), np.nan)
    lam2_opt = np.full(len(coef3_vec), np.nan)
    lam3_opt = np.full(len(coef3_vec), np.nan)
    acc_all_tune = np.full(K*len(coef3_vec), np.nan)
    acc_indpt_tune = np.full(K*len(coef3_vec), np.nan)
    obj_tune = np.full(K*len(coef3_vec), np.nan)
    ts_tune = np.full(K*len(coef3_vec), np.nan)
    acc_all_opt = np.full(K*len(coef3_vec), np.nan)
    acc_indpt_opt = np.full(K*len(coef3_vec), np.nan)
    obj_opt = np.full(K*len(coef3_vec), np.nan)
    ts_opt = np.full(K*len(coef3_vec), np.nan)
